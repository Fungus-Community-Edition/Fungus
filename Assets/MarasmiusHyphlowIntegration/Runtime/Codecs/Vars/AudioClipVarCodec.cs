...
namespace AtMycelia.Amanita.SaveSys
{
    ...
    public class AudioClipVarCodec : VarCodec, IVarCodec, IVarStateApplier<VariableSaveData>, IVarStateApplier<string>
    {
        ...
        private static ShadowDatabase ShadowDb
        {
            get
            {
                // If ShadowDb is null, find the first instance we can in Resources.
                // This way, we won't have to rely on something else to keep track of a ShadowDb.
                if (_shadowDb == null)
                {
                    _shadowDb = Resources.FindObjectsOfTypeAll<ShadowDatabase>()[0];
                }
